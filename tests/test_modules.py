...
def test_pooling_module_forward(mock_config: ESAConfig) -> None:
    ...
    pooling = PoolingModule(mock_config)
    z = torch.randn(8, 16, 32)  # batch_size=8, num_edges=16, n_embd=32

    out = pooling(z)
    assert out.shape == (8, 32), "PoolingModule output shape mismatch"
...
def test_esa_initialization(mock_config: ESAConfig) -> None:
    ...
    assert len(ab_blocks) == len(mock_config.layers), "Mismatch in layer initialization"
...
def test_esa_invalid_layers() -> None:
    ...
    config = ESAConfig(layers="SMSX")
    with pytest.raises(ValueError, match="`layers` should only contain 'S' and 'M'"):
        ESA(config)
